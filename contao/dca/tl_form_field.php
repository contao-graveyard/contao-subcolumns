 ...
 * Config
 */
$GLOBALS['TL_DCA']['tl_form_field']['config']['onsubmit_callback'][] = ['tl_form_subcols', 'scWrite'];
$GLOBALS['TL_DCA']['tl_form_field']['config']['ondelete_callback'][] = ['tl_form_subcols', 'scDelete'];
...
$GLOBALS['TL_DCA']['tl_form_field']['palettes']['formcolstart'] = '{type_legend},type;{colsettings_legend},fsc_type,fsc_color,fsc_name,fsc_equalize,fsc_gapuse;{expert_legend:collapsed},class';
...
$GLOBALS['TL_DCA']['tl_form_field']['fields']['fsc_parent'] = [
    'sql' => "int(10) unsigned NOT NULL default '0'",
];

$GLOBALS['TL_DCA']['tl_form_field']['fields']['fsc_childs'] = [
    'sql' => "varchar(255) NOT NULL default ''",
];

$GLOBALS['TL_DCA']['tl_form_field']['fields']['fsc_sortid'] = [
...
class tl_form_subcols extends tl_form_field
{
    ...
    {
        if ('formcolstart' !== $dc->activeRecord->type || '' === $dc->activeRecord->fsc_type) {
            return '';
        }
        ...
        if (1 === $dc->activeRecord->fsc_gapuse) {
    ...
    {
        if ('formcolstart' !== $dc->activeRecord->type || '' === $dc->activeRecord->fsc_childs) {
            return '';
        }
    ...
    public function scCopy($intId, DataContainer $dc): void
    {
        ...
        if ('copyAll' === $this->Input->get('act')) {
            ...
            if ('formcolstart' !== $objActiveRecord->type && 'formcolpart' !== $objActiveRecord->type && 'formcolend' !== $objActiveRecord->type) {
                $this->Database->prepare('UPDATE tl_form_field %s WHERE id=?')->set([
                    'fsc_name' => '',
                ])->execute($intId);
            }

            // Startelement mit aktuellen Daten besetzen und Session mit alten Daten füllen
            if ('formcolstart' === $objActiveRecord->type) {
                $arrSession = [
                    'parentId' => $intId,
                    'count' => 1,
                    'childs' => [],
            ...
            if ('formcolpart' === $objActiveRecord->type) {
            ...
            if ('formcolend' === $objActiveRecord->type) {
                $arrSession = $this->Session->get('sc'.$objActiveRecord->fsc_parent);
    ...
    private function moveRows($pid, $sorting, int $ammount = 128): void
    {
